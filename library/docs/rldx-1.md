...
6. **Filter A — video quality (Gemini API)** — two scores per clip: 16 uniformly sampled frames for _instruction following_, 8 frames for _trajectory plausibility_. Drops visually implausible or off-instruction generations before action labelling matters.
...
**Worth revisiting separately**: the synthetic-data pipeline above is architecture-agnostic and could feed _any_ Studio policy. Track as its own follow-up rather than as part of an RLDX-1 port.
